...
multinode-env:
	...
	vagrant up ipfs1
	vagrant ssh ipfs1 -- -t 'cd /mnt/vagrant && make ipfs-machine && make discover-ipfs-peers'
	vagrant up ipfs2
...
# ===================
define IPFS_DAEMON
echo $(shell ipfs daemon > /dev/null 2>&1 &) && sleep 2 && pidof ipfs
endef
...
discover-ipfs-peers: jq-command arp-scan-command
...
install-ipfs:
	rm -rf /tmp/go-ipfs*
	wget -P /tmp/ https://github.com/ipfs/go-ipfs/releases/download/v0.4.18/go-ipfs_v0.4.18_linux-amd64.tar.gz
	tar xvfz /tmp/go-ipfs_v0.4.18_linux-amd64.tar.gz -C /tmp
	rm /tmp/go-ipfs_v0.4.18_linux-amd64.tar.gz
...
kill-ipfs:
	sudo kill $(shell lsof -n -i :4001 | grep LISTEN | awk '{print $$2}' | uniq) || echo "No proceess listening on port 4001"
	sudo kill $(shell lsof -n -i :5001 | grep LISTEN | awk '{print $$2}' | uniq) || echo "No proceess listening on port 5001"
	sudo kill $(shell lsof -n -i :4040 | grep LISTEN | awk '{print $$2}' | uniq) || echo "No proceess listening on port 4040"
	sudo kill $(shell pidof ipfs) || echo "No IPFS program running"
# ===================
# API server
# ===================
...
install-ngrok:
	...
	unzip -q /tmp/ngrok-stable-linux-amd64.zip -d ~/fsbc
	rm -rf /tmp/ngrok*
...
# If dpkg --configure -a returns error Errors were encountered while processing: unattended-upgrades
# https://unix.stackexchange.com/questions/369561/unattended-upgrades-failing
clean-dpkg:
	sudo rm /var/lib/apt/lists/lock | echo "No /var/lib/apt/lists/lock found"
	sudo rm /var/cache/apt/archives/lock | echo "No /var/cache/apt/archives/lock found"
	sudo rm /var/lib/dpkg/lock | echo "No /var/lib/dpkg/lock found"
	sudo rm /var/lib/dpkg/lock-frontend | echo "No /var/lib/dpkg/lock-frontend found"
